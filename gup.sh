...
for img_file in out/target/product/veux/*.img; do
...
for zip_file in out/target/product/veux/*.zip; do
...
if [ "${DCDEVSPACE}" == "1" ]; then
    ...
    gh release create $RELEASETAG $ZIP_FILES $IMG_FILES $EXTRAFILES --repo release --title $RELEASETITLE --generate-notes
fi
